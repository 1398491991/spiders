...
class QfangwangSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        cityAreaName=normCityAreaName(cityAreaName.split(' ')[0] if cityAreaName else cityAreaName)
        ...
        items['communityCoverUrl']=communityCoverUrl.strip()  # 这个链接有 \n \r 等
    ...
    def communityHouseTypeParse(self,response):
        ...
        houseImgUrl_list=map(lambda x:re.sub('\d+x\d+','800x600',x.strip()),houseImgUrl_list)
        ...
        items['houseImgUrl_list'].append(houseImgUrl_list)
        items['houseType_list'].append(houseType_list)
        items['houseArea_list'].append(houseType_list)
        houseNextPage=response.xpath('//a[@class="turnpage_next fr"]/@href').extract_first()
        if houseNextPage:
            yield Request(houseNextPage,callback=self.communityHouseTypeParse,meta={'items':items})
        else:
            yield items